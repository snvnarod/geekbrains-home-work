...
def parse_params():
    ...
    parser.add_argument('--blocking', type=int, default=0,
...
def main():
    ...
    worker = {'client': src.socket_client.process,
              'server': lambda: src.socket_server.process(blocking=args.blocking)}.get(args.mode, lambda: print(
              '`{}`-mode not found'.format(args.mode)))
    worker()
